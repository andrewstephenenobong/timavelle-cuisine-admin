/* Timavelle content studio: server-backed drafts with explicit Save draft → Publish and confirmed removal. */
...
type DraftItem = { id: string; title: string; body: string; order: number; serverId?: string; key?: string };
...
function mapServerItem(kind: ContentKind, item: ServerItem, index: number): DraftItem {
  ...
  return { id: item._id || `${kind}-${index}`, serverId: item._id, key: kind === 'contact' ? item.key : undefined, title: title || '', body: body || '', order: item.order ?? index };
}
...
export default function ContentManager({ kind }: { kind: ContentKind }) {
  ...
  const [pendingRemoval, setPendingRemoval] = useState<DraftItem | null>(null);
  ...
  function beginEdit() { setEditingItems(cloneItems(items)); setPendingRemoval(null); setIsEditing(true); setDirty(false); setSaved(false); setError(''); }
  function cancelEdit() { setEditingItems(cloneItems(items)); setPendingRemoval(null); setIsEditing(false); setDirty(false); setError(''); }
  function updateItem(id: string, field: 'title' | 'body' | 'key', value: string) { setEditingItems((current) => current.map((item) => item.id === id ? { ...item, [field]: value } : item)); setDirty(true); setSaved(false); }
  function addItem() {
  ...
  function requestRemoval(item: DraftItem) {
    if (editingItems.length <= 1) { setError('Keep at least one content item in this area.'); return; }
    if (dirty) { setError('Save or cancel your other changes before deleting a content item.'); return; }
  ...
  async function confirmRemoval() {
    ...
    setError('');
    try {
      if (removed.serverId) await api.delete(`${API_ENDPOINTS[kind]}/${removed.serverId}`);
      const nextItems = editingItems.filter((item) => item.id !== removed.id);
      setItems(nextItems);
      setEditingItems(cloneItems(nextItems));
      ...
      setDirty(false);
      setSaved(true);
      setStatus('Draft deletion saved on the server. Publish to update the public site.');
    } catch (deleteError) {
      setError(apiErrorMessage(deleteError));
      setStatus('Draft deletion failed');
    } finally {
      setIsBusy(false);
    }
  }
  ...
  async function saveDraft() {
    ...
    try {
      ...
      setDirty(false);
      setSaved(true);
      setStatus('Draft saved on the server');
    } catch (saveError) {
  ...
  const publishDisabled = !apiReady || isBusy || isEditing || dirty || !saved || displayedItems.length === 0 || displayedItems.some((item) => !item.serverId);
  const publishTitle = !apiReady ? 'Connect the content API first' : displayedItems.length === 0 ? 'Keep at least one content item before publishing' : isEditing || dirty ? 'Save the draft before publishing' : !saved ? 'Save a draft before publishing' : 'Publish the saved draft to the public site';
  ...
  return <>
    <div className="content-studio">
      <div className="content-studio__head">
        ...
        <div className="content-studio__head-actions"><span className="content-studio__draft-status">{draftStatus}</span>{isEditing ? <div className="content-studio__edit-tools"><button className="content-studio__cancel" onClick={cancelEdit} disabled={isBusy}>Cancel</button><button className="content-studio__save" onClick={() => void saveDraft()} disabled={isBusy || !dirty}>Save draft ↗</button></div> : <button className="content-studio__edit" onClick={beginEdit} disabled={isBusy || !apiReady}>Edit content</button>}</div>
      </div>
      <div className="content-studio__notice"><span className="content-studio__notice-mark">i</span><div><strong>{apiReady ? (isEditing ? 'Editing a server draft' : 'Draft and publish mode') : 'Offline preview'}</strong><p>{meta.note} {isEditing ? 'Save or cancel other changes first; then use Delete and confirm to remove one draft immediately.' : 'Select Edit content before changing any field.'}</p></div><button className={`content-studio__publish${publishDisabled ? '' : ' content-studio__publish--ready'}`} onClick={() => void publish()} disabled={publishDisabled} title={publishTitle}>{isBusy ? 'WORKING…' : 'PUBLISH'}</button></div>
      <div className="content-studio__toolbar"><div><span className="admin-page__eyebrow">{displayedItems.length.toString().padStart(2, '0')} blocks in this page</span><h3>Build the public surface.</h3></div>{isEditing && <button className="content-studio__add" onClick={addItem}>+ Add {itemLabel}</button>}</div>
      <div className="content-studio__grid">{displayedItems.map((item, index) => <article className="content-studio__card" data-editing={isEditing} key={item.id}><div className="content-studio__card-top"><span>0{index + 1}</span>{isEditing && <button className="content-studio__remove" onClick={() => requestRemoval(item)} aria-label={`Delete ${item.title}`}>Delete</button>}</div>{isEditing ? <>{kind === 'contact' && <label>Public key<input value={item.key || ''} onChange={(event) => updateItem(item.id, 'key', event.target.value)} disabled={Boolean(item.serverId)} aria-describedby={item.serverId ? `contact-key-${item.id}` : undefined} />{item.serverId && <small id={`contact-key-${item.id}`}>The public key cannot be changed after creation.</small>}</label>}<label>Title<input value={item.title} onChange={(event) => updateItem(item.id, 'title', event.target.value)} /></label><label>Public copy<textarea value={item.body} onChange={(event) => updateItem(item.id, 'body', event.target.value)} rows={4} /></label></> : <><div className="content-studio__field"><span>Title</span><strong>{item.title}</strong></div><div className="content-studio__field"><span>Public copy</span><p>{item.body}</p></div></>}</article>)}</div>
      <div className="content-studio__footer"><span>API: <code>{API_ENDPOINTS[kind]}/admin</code> · save draft before publish</span>{isEditing && <button className="content-studio__add" onClick={addItem}>Add another {itemLabel}</button>}</div>
    </div>
    <ConfirmDialog open={Boolean(pendingRemoval)} title={`Delete ${pendingRemoval?.title || itemLabel}?`} message={`This ${itemLabel} will be removed from the server draft now. Published content stays unchanged until you publish the saved draft.`} confirmLabel="Delete draft" busy={isBusy} busyLabel="Deleting…" onCancel={() => setPendingRemoval(null)} onConfirm={() => void confirmRemoval()} />
  </>;
}
